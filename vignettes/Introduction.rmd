...
The mixture models used to cluster the drift patterns in `correctDrift()` can fail to converge for some combinations of geometry and cluster number. This results in missing Bayesian Information Criterion (BIC) measures for some models. You can check from which converged models the final model was selected using the "BIC" element in the output for basic data structures.
